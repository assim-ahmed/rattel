...
const Navbar = () => {
    ...
    return (
        <div className="fixed top-0 left-0 right-0 z-50 flex justify-center p-2">
            <nav
                className={`w-full max-w-7xl flex items-center justify-between px-6 py-3 transition-all duration-500 border z-50
                ${scrolled
            ...
            >
                ...
                <div className="flex items-center gap-3 group cursor-pointer">
                    ...
                    <h1 className="text-2xl font-black bg-gradient-to-l from-blue-600 to-indigo-500 bg-clip-text text-transparent dark:from-blue-400 dark:to-cyan-300">رتل</h1>
                </div>
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <Link
                                ...
                                ${active ? 'text-blue-600 dark:text-white' : 'text-slate-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-white'}`}
                            >
                                ...
                                <div className={`absolute inset-0 transition-transform duration-300 rounded-xl ${active ? 'bg-blue-50 dark:bg-blue-900/30 scale-100' : 'bg-slate-100 dark:bg-slate-800 scale-0 group-hover:scale-100'}`} />
                            </Link>
                        );
                    })}
                </div>
                ...
                <div className="flex items-center gap-3">
                    <button onClick={toggleTheme} className="w-11 h-11 flex items-center justify-center rounded-2xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-sm transition-all hover:-translate-y-1 active:scale-90">
                    ...
                    <button className="md:hidden w-11 h-11 flex items-center justify-center rounded-2xl bg-blue-600 text-white shadow-xl shadow-blue-500/40 active:scale-90 z-50" onClick={() => setIsOpen(!isOpen)}>
            ...
            {/* ✅ الحل هنا: التبديل بين pointer-events-none و pointer-events-auto بناءً على isOpen */}
            <div 
                className={`fixed inset-0 top-0 pt-[88px] p-4 md:hidden transition-all duration-500 z-40
                ${isOpen 
                    ? 'opacity-100 translate-y-0 pointer-events-auto bg-black/20 backdrop-blur-sm' 
                    : 'opacity-0 -translate-y-10 pointer-events-none'}`}
                onClick={() => setIsOpen(false)}
            >
                <div 
                    className="w-full bg-white/90 dark:bg-slate-900/90 backdrop-blur-2xl rounded-[32px] border border-white/20 dark:border-slate-800 p-6 shadow-2xl"
                    onClick={(e) => e.stopPropagation()} // منع الانغلاق عند الضغط داخل القائمة
                >
                        ...
                        {navItems.map((item) => {
                            ...
                            return (
                                <Link 
                                    ...
                                    ${active ? 'bg-blue-100 dark:bg-blue-900/40 border border-blue-200' : 'bg-slate-50 dark:bg-slate-800/50'}`}
                                >
                                    <span className={`text-3xl ${active ? 'scale-110' : ''}`}>
                                        {item.icon(active ? '#2563eb' : (theme === 'dark' ? '#94a3b8' : '#1f2d56'))}
                                    </span>
